...
public class GroupPaymentCommandHandler implements CommandHandler<GroupPaymentCommand> {
    ...
    public void handle(GroupPaymentCommand command) {
        ...
        Currency currencyForPayment = group.getCurrency();
        ...
        for(GroupMember member : groupMembersWithoutAdmin){
            this.threadRunner.run(() -> {
                makePaymentGroupMemberToAdmin(group, moneyToPayPerMember, currencyForPayment, member);
    ...
    private void makePaymentGroupMemberToAdmin(Group group, double moneyToPayPerMember, Currency currencyCodeForPayment, GroupMember member) {
        try {
