...
public interface UsingGroups extends UsingGroupMembers{
    ...
    default void addGroup(UUID groupId, UUID adminUserId, double money, UUID... membersUserId){
        this.groupRepository().save(new Group(groupId, DEFAULT_DESCRIPTION, LocalDateTime.now(), money, adminUserId, GroupState.PROCESS, Currency.EUR));

        addMember(groupId, adminUserId, GroupMemberRole.ADMIN);
        addMember(groupId, membersUserId);
    }

    default void addGroup(UUID groupId, GroupState groupState){
        UUID adminUserId = UUID.randomUUID();
        this.groupRepository().save(new Group(groupId, DEFAULT_DESCRIPTION, LocalDateTime.now(), 10, adminUserId, groupState, Currency.EUR));

        addMember(groupId, adminUserId, GroupMemberRole.ADMIN);
    }

    default void addGroup(UUID groupId, UUID userId){
        this.groupRepository().save(new Group(groupId, DEFAULT_DESCRIPTION, LocalDateTime.now(), 1, userId, GroupState.PROCESS, Currency.EUR));

        addMember(groupId, userId, GroupMemberRole.ADMIN);
    }

    default void addGroup(UUID groupId, UUID userId, double money){
        this.groupRepository().save(new Group(groupId, DEFAULT_DESCRIPTION, LocalDateTime.now(), money, userId, GroupState.PROCESS, Currency.EUR));
